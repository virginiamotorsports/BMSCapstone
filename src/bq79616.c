...
int ReadReg(char bID, uint16_t wAddr, char * pData, char bLen, uint32_t dwTimeOut,
    ...
    if (bWriteType == FRMWRT_SGL_R) {
        ...
        // BQ_board.readBytes(pData, bLen + 6);
        ...
        // BQ_board.readBytes(pData, (bLen + 6) * (TOTALBOARDS - 1));
        while(UART_RX_RDY == 0U && count>0) count--; /* Wait */
        UART_RX_RDY = 0;
        ...
        // BQ_board.readBytes(pData, (bLen + 6) * TOTALBOARDS);
        while(UART_RX_RDY == 0U && count>0) count--; /* Wait */
        UART_RX_RDY = 0;
